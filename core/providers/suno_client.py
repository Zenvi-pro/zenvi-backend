...
Provides music generation via the Suno API.
This module is logic-only (no Qt).
"""
...
DEFAULT_BASE_URL = "https://studio-api.prod.suno.com/api/v2/external/"
...
def _raise_for_status(resp) -> None:
    ...
    if status == 401:
        raise SunoError("Authentication failed. Check your Suno token.", status_code=status, detail=detail)
    elif status == 403:
        raise SunoError("Access denied. Check your Suno API permissions.", status_code=status, detail=detail)
    elif status == 429:
        raise SunoError("Rate limit exceeded. Please wait.", status_code=status, detail=detail)
    elif status == 400:
        raise SunoError("Bad request. Check parameters.", status_code=status, detail=detail)
    else:
...
def suno_generate(
    *,
    ...
    """POST /generate — returns the generated clip object."""
...
def suno_get_clips(
    *,
    ...
    """GET /clips?ids=... — returns array of clip objects."""
...
def suno_wait_for_clip(
    *,
    ...
    """Poll /clips until a clip reaches a target status."""
...
def download_mp3(
    *,
    ...
    """Download an MP3 from a URL to dest_path. Returns (ok, error_message)."""
...
def generate_wait_download_mp3(
    *,
    ...
    clip = suno_generate(
        token=token, topic=topic, tags=tags, negative_tags=negative_tags,
        prompt=prompt, make_instrumental=make_instrumental,
        cover_clip_id=cover_clip_id, base_url=base_url, timeout_seconds=60.0,
    )
    cid = str(clip.get("id") or "").strip()
    final_clip = suno_wait_for_clip(
        token=token, clip_id=cid, base_url=base_url,
        timeout_seconds=float(timeout_seconds), poll_seconds=float(poll_seconds),
        target_statuses=("complete",), allow_streaming=True,
    )
